...
function mapPrismaError(error) {
    if (error instanceof Prisma.PrismaClientKnownRequestError) {
        ...
        switch (error.code) {
            case "P2002":
                return new ApiError(409, "Unique constraint failed", [{ code: error.code, target: error.meta?.target }], error.stack);
            case "P2025":
                return new ApiError(404, "Record not found", [{ code: error.code }], error.stack);
            case "P2003":
                return new ApiError(409, "Foreign key constraint failed", [{ code: error.code, field: error.meta?.field_name }], error.stack);
            default:
