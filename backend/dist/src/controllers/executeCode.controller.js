...
import { getLanguageName, pullBatchResults, submitBatch, } from "../utils/lib/judge0.js";
...
export const executeCode = asyncHandler(async (req, res) => {
    ...
    if (!source_code || !language_id || !problemId) {
        throw new ApiError(400, "source_code, language_id and problemId are required", []);
    }
    ...
    if (!Array.isArray(expected_outputs) ||
        ...
        throw new ApiError(400, "expected_outputs must be an array with the same length as stdin", []);
    }
    ...
    const detailedResults = results.map((result, index) => {
        const { stdout: actualOutput, time, memory, stderr, compile_output, status, } = result;
        const stdout = actualOutput?.trim() ?? "";
        const expectedOutput = expected_outputs[index]?.trim() ?? "";
        const isTestCasePassed = stdout.replace(/\r\n/g, "\n") === expectedOutput.replace(/\r\n/g, "\n");
        if (!isTestCasePassed) {
    ...
    const normalizedSubmission = submissionWithTestCases
        ...
        : null;
    res.status(201).json(new ApiResponse(201, "Code executed successfully", normalizedSubmission));
});
